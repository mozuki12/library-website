<!DOCTYPE html>
    ...
    <body>
      <div id="app">
        <nav class="navbar navbar-expand-md navbar-light shadow-sm bg-nav">
            <div class="container">
                    ...
                    <ul class="navbar-nav">
                        ...
                        @else
                            ...
                            <form class="form-inline my-2 my-lg-0" action="{{ route('search') }}" method="GET">
                              <input class="form-control mr-sm-2" type="search" id="search-saya" name="query" placeholder="Search" aria-label="Search">
                          ...
                          @foreach ($books as $index => $book)
                            <div class="card rounded mt-4" style="width: 16rem;"  >
                              ...
                              <form method="POST" action="{{ route('home.destroy', $book->id) }}">
                                @csrf
                                @method('DELETE')
                                <div class="card-body bg-radius">
                                  <p id="font-family">pengarang: <b>{{$book-> pengarang}}</b></p>
                                  <p id="font-family">penerbit: <b>{{$book-> penerbit}}</b></p>
                                </div>
                                <div class="card-body m-1">
                                    <a href="{{route('add.edit',  $book->id)}}"><button type="button" class="btn btn-warning">edit</button></a>
